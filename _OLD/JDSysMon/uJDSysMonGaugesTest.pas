...
type
  TfrmJDSSysMonGaugesTest = class;

  TDriveGaugeRef = class(TObject)
  private
    FOwner: TfrmJDSSysMonGaugesTest;
    FDrive: TJDSystemMonitorDriveInfo;
    FGauge: TJDGauge;
  public
    constructor Create(Owner: TfrmJDSSysMonGaugesTest; Drive: TJDSystemMonitorDriveInfo);
  ...
  TfrmJDSSysMonGaugesTest = class(TfrmContentBase)
    gCPU: TJDGauge;
    gRAM: TJDGauge;
...
constructor TDriveGaugeRef.Create(Owner: TfrmJDSSysMonGaugesTest; Drive: TJDSystemMonitorDriveInfo);
begin
  FOwner:= Owner;
  FDrive:= Drive;
  //TODO: Create gauge...
  FGauge:= TJDGauge.Create(nil);
  FGauge.Parent:= FOwner;
  FGauge.GaugeType:= 'Horizontal Bar';
  FGauge.Assign(FOwner.gCPU);
  FGauge.Align:= alTop;
  FGauge.Top:= 50000;
  FOwner.FormResize(nil);
  FOwner.AdjustColors;
end;
...
begin
  if Application.Terminated then Exit;
  R:= TDriveGaugeRef.Create(Self, Drive);
  try
