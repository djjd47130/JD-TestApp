...
begin
  ...
  try
    ...
    //If AID is specified, and identical tab already exists,
    //  prompt whether to open existing tab or create new tab.
    if AID <> -1 then begin
      var Tmp:= TabByID(AID);
      if Assigned(Tmp) then begin
        if Tmp.FClass = AClass then begin
          //TODO: Prompt whether to open this one instead...
          if MessageDlg('Tab "'+Tmp.Caption+'" is already opened.'+
            sLineBreak+sLineBreak+
            'Jump to that instead of creating new tab?',
